...
\begin{itemize}
    ...
    % \item \textbf{Hypercube Basis:} A 4-dimensional algebraic basis in $\mathbb{Q}^4$ mapped to the Euclidean $\mathbb{R}^2$ plane. A vector $\mathbf{v} = [x, y, z, w]^T$ represents the physical 2D coordinate $\mathbf{p}$ via the mapping: $\mathbf{p} = x \mathbf{b}_1 + y \mathbf{b}_2 + z \mathbf{b}_3 + w \mathbf{b}_4$, where the basis vectors are $\mathbf{b}_1 = [1,0]^T$, $\mathbf{b}_2 = [s,s]^T$, $\mathbf{b}_3 = [0,1]^T$, and $\mathbf{b}_4 = [-s,s]^T$ with $s = \sqrt{2}/2$.
\end{itemize}
...
To intersect creases formed by vertices $\mathbf{v}_a$ and $\mathbf{v}_b$ at angles $\alpha$ and $\beta$, we utilize precomputed rational tangents. The intersection vertex $\mathbf{v}_c$ is calculated as:
\begin{align}
...
\subsection{Basic Geometric Constraints}
...
% The quadruplet constraint adds the following constraint as a row to the global constraint matrix $A$:
% \begin{equation}
%     \begin{bmatrix}
%     \dots & (w_1 n_{1x}) & (w_1 n_{1y}) & \dots & (w_4 n_{4x}) & (w_4 n_{4y}) & \dots
%     \end{bmatrix}
%     \begin{bmatrix}
%     \vdots \\
%     x_1 \\
...
\subsection{Quadruplet selection via MILP}
...
\begin{equation}
    -C(1 - z_k) \le \mathbf{n} \cdot (\mathbf{P}_k - \mathbf{p}_u) - r_k \le C(1 - z_k)
\end{equation}
...
\end{equation}
\begin{align}
    \mathbf{n} \cdot (\mathbf{p}_c - \mathbf{p}_u) &\ge \text{gap} - C(1 - b_1) \\
    \mathbf{t} \cdot (\mathbf{p}_c - \mathbf{p}_u) &\le -\text{gap} + C(1 - b_2) \\
    \mathbf{t} \cdot (\mathbf{p}_c - \mathbf{p}_v) &\ge \text{gap} - C(1 - b_3)
\end{align}
...
\textcolor{red}{[Figure Placeholder: A visual diagram comparing three distinct MILP tilings generated from the exact same unweighted topology. Highlight the specific quadruplets chosen in each iteration and how they radically alter the placement of the interior straight skeleton vertices.]}
...
Consider a standard angular constraint requiring the edge between node $u$ and node $v$ to remain parallel to a known 22.5$^\circ$ directional vector $\mathbf{d} = (d_x, d_y)$. The 2D cross product must be exactly zero:
\begin{equation}
